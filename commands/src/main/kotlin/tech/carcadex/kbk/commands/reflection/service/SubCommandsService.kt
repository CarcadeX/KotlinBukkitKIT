...
import tech.carcadex.kbk.commands.reflection.annotations.Default
...
class SubCommandsService(private val messagesService: MessagesService, private val tabCompleteService: TabCompleteService) {
    ...
    fun registerRootCommand(command: Any): Pair<String, List<SubCommand>> {
        ...
        for(func in clazz.functions) {
            if(func.hasAnnotation<tech.carcadex.kbk.commands.reflection.annotations.SubCommand>()) {
    ...
    private fun parseSubCommand(func: KFunction<*>, rootCommand: Any): SubCommand {
        ...
        val name = if(func.hasAnnotation<Default>()) "default" else func.findAnnotations(tech.carcadex.kbk.commands.reflection.annotations.SubCommand::class).first().name
