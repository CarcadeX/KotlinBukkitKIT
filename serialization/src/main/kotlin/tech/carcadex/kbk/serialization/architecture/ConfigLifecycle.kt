...
package tech.carcadex.kbk.serialization.architecture

import tech.carcadex.kbk.architecture.KotlinPlugin
import tech.carcadex.kbk.architecture.lifecycle.LifecycleListener
import tech.carcadex.kbk.serialization.KotlinConfigEvent
import tech.carcadex.kbk.serialization.SerializationConfig
import tech.carcadex.kbk.serialization.architecture.impl.ConfigDelegate
import tech.carcadex.kbk.serialization.architecture.impl.getOrInsertConfigLifecycle
import tech.carcadex.kbk.serialization.architecture.impl.registerConfiguration
...
): tech.carcadex.kbk.serialization.SerializationConfig<T> {
    val configFile = File(dataFolder, file)

    return tech.carcadex.kbk.serialization.SerializationConfig(
        defaultModel,
        configFile,
        serializer,
        type,
        ...
        eventObservable = {
            if (it == tech.carcadex.kbk.serialization.KotlinConfigEvent.RELOAD) {
                someConfigReloaded()
            }
        },
    ).also {
        registerConfiguration(it as tech.carcadex.kbk.serialization.SerializationConfig<Any>, loadOnEnable, saveOnDisable)
    }
}
 ...
 */
public fun LifecycleListener<*>.getConfig(type: KType): tech.carcadex.kbk.serialization.SerializationConfig<*> {
    try {
