...
package tech.carcadex.kotlinbukkitkit.serialization.architecture
...
import tech.carcadex.kotlinbukkitkit.serialization.KotlinConfigEvent
import tech.carcadex.kotlinbukkitkit.serialization.SerializationConfig
import tech.carcadex.kotlinbukkitkit.serialization.architecture.impl.ConfigDelegate
import tech.carcadex.kotlinbukkitkit.serialization.architecture.impl.getOrInsertConfigLifecycle
import tech.carcadex.kotlinbukkitkit.serialization.architecture.impl.registerConfiguration
...
): tech.carcadex.kotlinbukkitkit.serialization.SerializationConfig<T> {
    val configFile = File(dataFolder, file)

    return tech.carcadex.kotlinbukkitkit.serialization.SerializationConfig(
        defaultModel,
        configFile,
        serializer,
        type,
        ...
        eventObservable = {
            if (it == tech.carcadex.kotlinbukkitkit.serialization.KotlinConfigEvent.RELOAD) {
                someConfigReloaded()
            }
        },
    ).also {
        registerConfiguration(it as tech.carcadex.kotlinbukkitkit.serialization.SerializationConfig<Any>, loadOnEnable, saveOnDisable)
    }
}
 ...
 */
public fun LifecycleListener<*>.getConfig(type: KType): tech.carcadex.kotlinbukkitkit.serialization.SerializationConfig<*> {
    try {
